Partial Class GF_qcmRequestAllotment
  Inherits SIS.SYS.GridBase
	...
	End Function
	Protected Sub GVqcmRequestAllotment_RowCommand(ByVal sender As Object, ByVal e As System.Web.UI.WebControls.GridViewCommandEventArgs) Handles GVqcmRequestAllotment.RowCommand
		If e.CommandName.ToLower = "lgedit".ToLower Then
			Try
				Dim RequestID As Int32 = GVqcmRequestAllotment.DataKeys(e.CommandArgument).Values("RequestID")
				Dim RedirectUrl As String = TBLqcmRequestAllotment.EditUrl & "?RequestID=" & RequestID
				Response.Redirect(RedirectUrl)
			Catch ex As Exception
			End Try
		End If
    If e.CommandName.ToLower = "lgsendsms".ToLower Then
      Try
        Dim RequestID As Int32 = GVqcmRequestAllotment.DataKeys(e.CommandArgument).Values("RequestID")
        SIS.QCM.qcmRequestAllotment.SendSMSandEMail(RequestID)
      Catch ex As Exception
      End Try
    End If
    If e.CommandName.ToLower = "lgreturn".ToLower Then
      Try
        Dim RequestID As Int32 = GVqcmRequestAllotment.DataKeys(e.CommandArgument).Values("RequestID")
        Dim Remarks As String = CType(GVqcmRequestAllotment.Rows(e.CommandArgument).FindControl("F_ReturnRemarks"), TextBox).Text
        SIS.QCM.qcmRequests.ReturnToRequester(RequestID, Remarks)
        GVqcmRequestAllotment.DataBind()
      Catch ex As Exception
      End Try
    End If
  End Sub
  Protected Sub GVqcmRequestAllotment_Init(ByVal sender As Object, ByVal e As System.EventArgs) Handles GVqcmRequestAllotment.Init
    DataClassName = "GqcmRequestAllotment"
